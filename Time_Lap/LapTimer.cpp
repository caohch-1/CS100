...
LapHandle::LapHandle(int number) {
  if (number < 0)
    m_number = -1;
  m_number = number;
}
...
LapTimer::LapTimer( bool start, size_t precision ) {
...
void
LapTimer::stop( bool restart ) {
  int iterations = 1;
  stop(iterations,restart);
}

void
LapTimer::stop( size_t iterations, bool restart ) {
  stop( (int) iterations, restart );
}

void
LapTimer::stop( int iterations, bool restart ) {
  if (!m_running) {
...
void
LapTimer::stop( LapHandle & nextLap, bool restart ) {
  int iterations = 1;
  stop( nextLap, iterations, restart );
}

void
LapTimer::stop( LapHandle & nextLap, size_t iterations, bool restart ) {
  stop( nextLap, (int) iterations, restart );
}

void
LapTimer::stop( LapHandle & nextLap, int iterations, bool restart ) {
  if (!m_running) {
    std::cout << "Error: timer is not started\n";
    return;
  }
...
LapHandle
LapTimer::addLap( const std::string & lapName ) {
...
LapHandle
LapTimer::findLap( const std::string & lapName ) const {
  int index = -1;
